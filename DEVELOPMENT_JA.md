...
現在Loot計算が正確に実装されているのは上記4種類です。ジャングルの寺院や
...
### 48-bit familyのLoot高速化

検索方式を`48-bit family blocks`にすると、検索ボタン横の
`Loot高速化（48-bit family）`を選択できます。既定はOFFです。

ONの場合も、構造物の生成可否、バイオーム、荒廃したポータルの地下・
ジャングル等のバリアント条件は、上位16bitごとに通常どおり判定します。
再利用するのはチェスト内容の計算だけです。
...
現対応範囲では精度を落とさない高速化です。下位48bitが次のfamilyへ変わると
キャッシュを破棄するため、メモリは一家族分に限定されます。設定はSessionの
`#FastLoot48: 0/1`へ保存され、ヘッドレス検索でも同じ動作になります。
