...
void AnalysisStructures::run()
{
    ...
    if (seeds.size() > 1)
    {
        ...
        const size_t workerCount = std::min<size_t>(seeds.size(), hw ? hw : 1);
...
void AnalysisStructures::runStructs(Generator *g)
{
    ...
    if (!shouldStop() && mapshow[D_SPAWN] && (dim == DIM_UNDEF || dim == DIM_OVERWORLD))
    {
    ...
    if (!shouldStop() && mapshow[D_STRONGHOLD] && (dim == DIM_UNDEF || dim == DIM_OVERWORLD))
    {
        StrongholdIter sh;
