...
{
    QVariant stored = settings.value("map/mc", mc2str(mc));
    QByteArray version = stored.toString().toLatin1();
    int parsed = str2mc(version.constData());
    if (parsed != MC_UNDEF)
    {
        mc = parsed;
    }
    ...
    {
        // 4.1.x stored raw enum values. The two newest values moved when
        // 1.21.2 was restored as a distinct stable release.
        int legacy = stored.toInt();
        if (legacy == 27)
            mc = MC_1_21_3;
        else if (legacy == 28)
            mc = MC_1_21_4;
        else if (legacy > MC_UNDEF && legacy <= 26)
            mc = legacy;
    }
...
{
    settings.setValue("map/mc", mc2str(mc));
...
QString mapopt2display(int opt)
{
    ...
    case D_GEODE:       return QApplication::translate("Map", "Amethyst Geode");
...
void MapConfig::reset()
{
    ...
    opts[D_OREVEIN].scale = 16;
    opts[D_MANSION].scale = 128;
    zoomEnabled = false;
}
...
{
    reset();
    const bool migrateMansionRange = !settings.value("structscale/mansion_range_v2", false).toBool();
    for (int opt = 0; opt < D_STRUCT_NUM; opt++)
    {
        ...
        if (opt == D_MANSION && migrateMansionRange && fabs(x) == 32.0)
            x = copysign(128.0, x);
        opts[opt].scale = fabs(x);
        opts[opt].enabled = x > 0;
    }
    zoomEnabled = settings.value("structscale/zoom", zoomEnabled).toBool();
    if (migrateMansionRange)
        settings.setValue("structscale/mansion_range_v2", true);
}
