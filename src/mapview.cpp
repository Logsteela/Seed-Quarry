    ...
    , config()
{
    ...
    QFont fmono = QFontDatabase::systemFont(QFontDatabase::FixedFont);
    setFont(fmono);
...
void MapView::refresh()
{
    if (world)
    {
        ...
        world->setBiomeHighlights(biomeHighlights);
    }
}
...
void MapView::setBiomeHighlight(int biomeId)
{
...
void MapView::setBiomeHighlights(const QVector<int>& biomeIds)
{
    biomeHighlights = biomeIds;
    if (world)
    {
        world->setBiomeHighlights(biomeIds);
        update(1);
    }
}
...
{
    QWidget *coordinateFinder = findChild<QWidget*>("mapCoordinateFinder", Qt::FindDirectChildrenOnly);
    bool coordinateFinderVisible = coordinateFinder && coordinateFinder->isVisible();
    overlay->setVisible(false);
    if (coordinateFinder)
        coordinateFinder->setVisible(false);
    QPixmap img = grab();
    overlay->setVisible(true);
    if (coordinateFinderVisible)
        coordinateFinder->setVisible(true);
    return img;
}
...
void MapView::resizeEvent(QResizeEvent *e)
{
    ...
    if (coordinateFinder)
    {
        coordinateFinder->move(qMax(14, (width() - coordinateFinder->width()) / 2), 14);
