...
public:
    explicit AnalysisBiomes(QObject *parent = nullptr)
        : QThread(parent),idx(),locateDone(),locateTotal() {}
...
public:
    ...
    std::atomic_long idx;
    std::atomic_ulong locateDone;
    std::atomic_ulong locateTotal;
    int dims[3];
    struct Dat {
        int x1, z1, x2, z2;
        int scale;
...
public:
    ...
    QVector<int> selectedBiomeList() const;
...
private slots:
    ...
    void on_comboBiome_selectionChanged();
    void on_buttonHighlight_toggled(bool checked);
