...
enum { ORE_VEIN_COPPER = 10001, ORE_VEIN_IRON = 10002 };
enum { VEIN_NONE, VEIN_COPPER, VEIN_IRON };
...
static Xoroshiro oreRandomAt(const Xoroshiro& base, int x, int y, int z)
{
    int64_t l = (int64_t)x * 3129871LL ^ (int64_t)z * 116129781LL ^ (int64_t)y;
    l = l * l * 42317861LL + l * 11LL;
    l >>= 16;
    return Xoroshiro{(uint64_t)l ^ base.lo, base.hi};
}

static double clampedMapValue(double v, double in0, double in1, double out0, double out1)
{
    double t = (v - in0) / (in1 - in0);
    if (t < 0) t = 0;
    if (t > 1) t = 1;
    return out0 + t * (out1 - out0);
}

// Vanilla 1.18+ ore-vein noises. The parameter names and constants mirror
// Mojang's worldgen noise registry; the implementation uses cubiomes' noise.
static bool initOreVeinNoise(OreVeinNoise& n, uint64_t seed, int mc)
{
    if (mc < MC_1_18)
        return false;

    static const uint64_t md5Ore[2] = {0x9b88124de600116d, 0x2ae68055aa4a7761};
    static const uint64_t md5Veininess[2] = {0x6b86c7820a307171, 0xd87fb0fefd9c1624};
    static const uint64_t md5VeinA[2] = {0x4cd8d69b9a841649, 0xcdd63f17bfe8f5ed};
    static const uint64_t md5VeinB[2] = {0x6b26220b31f7c6c9, 0xae077edebf6aaec1};
    static const uint64_t md5Gap[2] = {0x9c4cc6b2fb0be4bb, 0xbd5964705573bb5e};
    static const double amplitude[] = {1.0};

    Xoroshiro world;
    xSetSeed(&world, seed);
    uint64_t lo = xNextLong(&world);
    uint64_t hi = xNextLong(&world);

    Xoroshiro ore = {lo ^ md5Ore[0], hi ^ md5Ore[1]};
    n.positional = Xoroshiro{xNextLong(&ore), xNextLong(&ore)};

    int used = 0;
    Xoroshiro xr = {lo ^ md5Veininess[0], hi ^ md5Veininess[1]};
    used += xDoublePerlinInit(&n.veininess, &xr, n.oct + used, amplitude, -8, 1, -1);
    xr = Xoroshiro{lo ^ md5VeinA[0], hi ^ md5VeinA[1]};
    used += xDoublePerlinInit(&n.veinA, &xr, n.oct + used, amplitude, -7, 1, -1);
    xr = Xoroshiro{lo ^ md5VeinB[0], hi ^ md5VeinB[1]};
    used += xDoublePerlinInit(&n.veinB, &xr, n.oct + used, amplitude, -7, 1, -1);
    xr = Xoroshiro{lo ^ md5Gap[0], hi ^ md5Gap[1]};
    used += xDoublePerlinInit(&n.gap, &xr, n.oct + used, amplitude, -5, 1, -1);
    return used <= 8;
}

static int oreVeinAt(int x, int y, int z, OreVeinNoise& n)
{
    double ridge = fmax(fabs(sampleDoublePerlin(&n.veinA, 4*x, 4*y, 4*z)),
                        fabs(sampleDoublePerlin(&n.veinB, 4*x, 4*y, 4*z)));
    if (ridge >= 0.08)
        return VEIN_NONE;

    double toggle = sampleDoublePerlin(&n.veininess, x*1.5, y*1.5, z*1.5);
    const bool copper = toggle > 0;
    const int minY = copper ? 0 : -60;
    const int maxY = copper ? 50 : -8;
    if (y < minY || y > maxY)
        return VEIN_NONE;

    double density = fabs(toggle);
    int edge = std::min(maxY-y, y-minY);
    if (density + clampedMapValue(edge, 0, 20, -0.2, 0) < 0.4)
        return VEIN_NONE;

    Xoroshiro random = oreRandomAt(n.positional, x, y, z);
    if (xNextFloat(&random) > 0.7f)
        return VEIN_NONE;

    // Include both ore and its granite/tuff filler: both identify the same
    // large vein body, while the exact exposed blocks depend on terrain.
    (void) sampleDoublePerlin(&n.gap, x, y, z);
    return copper ? VEIN_COPPER : VEIN_IRON;
}

static void getOreVeins(std::vector<VarPos>& out, WorldInfo wi,
                        int x0, int z0, int x1, int z1)
{
    OreVeinNoise noise;
    if (!initOreVeinNoise(noise, wi.seed, wi.mc))
        return;

    const int markerSize = 128;
    int mx0 = (int)floor((double)x0 / markerSize);
    int mz0 = (int)floor((double)z0 / markerSize);
    int mx1 = (int)ceil((double)x1 / markerSize);
    int mz1 = (int)ceil((double)z1 / markerSize);
    for (int mz = mz0; mz < mz1; mz++)
    for (int mx = mx0; mx < mx1; mx++)
    {
        bool found = false;
        for (int cz = 0; cz < 8 && !found; cz++)
        for (int cx = 0; cx < 8 && !found; cx++)
        {
            int bx = mx*markerSize + cx*16;
            int bz = mz*markerSize + cz*16;
            Xoroshiro pick = oreRandomAt(noise.positional, bx, 0, bz);
            int x = bx + xNextInt(&pick, 16);
            int z = bz + xNextInt(&pick, 16);
            for (int y = -60; y <= 50; y += 4)
            {
                int vein = oreVeinAt(x, y, z, noise);
                if (vein == VEIN_NONE)
                    continue;
                VarPos vp(Pos{x, z}, vein == VEIN_COPPER ? ORE_VEIN_COPPER : ORE_VEIN_IRON);
                vp.v.y = y;
                vp.v.biome = vein;
                out.push_back(vp);
                found = true;
...
QStringList VarPos::detail() const
{
    QStringList sinfo;
    QString s;
    if (type == ORE_VEIN_COPPER || type == ORE_VEIN_IRON)
    {
        sinfo.append(type == ORE_VEIN_COPPER ? "copper" : "iron");
...
Quad::Quad(const Level* l, int64_t i, int64_t j)
    ...
    , ti(i),tj(j),blocks(l->blocks),pixs(l->pixs),sopt(l->sopt),biomeHighlights(l->world->biomeHighlights)
    , biomes(),rgb(),img(),spos()
{
    isdel = l->isdel;
    setAutoDelete(false);
...
void Quad::updateBiomeColor()
{
    ...
    else
    {
        ...
        if (biomeHighlights.any())
        {
            for (int i = 0; i < r.sx * r.sz; i++)
            {
                uchar *col = rgb + 3*i;
                int biomeId = biomes[i];
                if (biomeId >= 0 && biomeId < 256 && biomeHighlights.test(biomeId))
                {
                    // Keep the selected biome vivid and slightly brighter.
                    for (int k = 0; k < 3; k++)
                        col[k] = (uchar) std::min(255, 24 + (int) col[k]);
                }
                else
                {
                    // Chunkbase-style focus: preserve terrain context while
                    // reducing every non-matching biome to a dark neutral gray.
                    int gray = (54*col[0] + 183*col[1] + 19*col[2]) >> 8;
                    gray = 18 + gray / 5;
                    col[0] = col[1] = col[2] = (uchar) gray;
                }
            }
        }

        // End biomes describe large regions and do not distinguish terrain
        // from the void. At the detailed map levels, use the actual End
        // surface to reveal the main island and the shapes of outer islands.
        if (dim == DIM_END && r.scale <= 8)
        {
            std::vector<float> height(r.sx * r.sz);
            if (mapEndSurfaceHeight(height.data(), &g->en, sn,
                    r.x, r.z, r.sx, r.sz, r.scale, 0) == 0)
            {
                mapEndIslandHeight(height.data(), &g->en, g->seed,
                    r.x, r.z, r.sx, r.sz, r.scale);
                for (int i = 0; i < r.sx * r.sz; i++)
                {
                    if (height[i] <= 0)
                    {
...
void Quad::run()
{
    ...
    else if (pixs < 0)
    {
        if (sopt == D_OREVEIN)
        {
            ...
            getOreVeins(*st, wi, ti*blocks, tj*blocks, (ti+1)*blocks, (tj+1)*blocks);
            spos = st;
            done = true;
            return;
        }
...
void QWorld::setBiomeHighlight(int biomeId)
{
    QVector<int> biomeIds;
    if (biomeId >= 0)
        biomeIds.append(biomeId);
    setBiomeHighlights(biomeIds);
}

void QWorld::setBiomeHighlights(const QVector<int>& biomeIds)
{
    std::bitset<256> highlights;
    for (int biomeId : biomeIds)
    {
        if (biomeId >= 0 && biomeId < 256)
            highlights.set(biomeId);
    }
    if (biomeHighlights == highlights)
        return;

    waitForIdle();
    QMutexLocker locker(&g_mutex);
    biomeHighlights = highlights;
    for (Level& l : lvb)
    {
        for (Quad *q : l.cells)
        {
            q->biomeHighlights = highlights;
            q->updateBiomeColor();
        }
    }
    for (Quad *q : cachedbiomes)
